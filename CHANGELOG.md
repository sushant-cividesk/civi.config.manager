...
- Hardened targeted Playwright QA for Drupal Buildkit/DDEV: authenticate only through `/user/login`, prove a Drupal session cookie before CiviCRM access checks, distinguish bad login from missing Configuration Manager permission, and fail early with the Chromium install command when the browser binary is missing.
...
- Preserved compatibility with alpha61/alpha62 CiviRules YAML that predates `identity_portable`; absence of the newer metadata no longer silently converts otherwise portable legacy configuration into monitor-only state.
- Moved durable queued-operation workspaces to CiviCRM `ConfigAndLog` when available so SQL-backed jobs normally survive PHP/container restarts; system temp remains a fallback for early-bootstrap/test runtimes.
- Reworked ambiguous identity handling so duplicate/unproven CiviRules and contributed-provider rows are exported as deterministic **monitor-only snapshots** instead of blocking the entire Export. Identical duplicates receive stable fingerprint/occurrence paths such as `name--ambiguous-<fingerprint>-01.yml`; local database IDs are never portable identity.
- Added explicit ambiguity metadata (`group_count`, `content_count`, canonical content fingerprint, occurrence) so Synchronize can compare repeated ambiguous content as a stable multiset. Intentional monitor-only source rows are skipped during Import while unrelated safe identities may continue; a YAML item proven portable at source which matches multiple target rows remains a blocking preflight conflict.
- Made delete-missing safety per identity for CiviRules and discovered extension providers: an ambiguous identity is never deleted automatically, but it no longer disables safe cleanup for every unrelated unique identity in the same provider. CiviRules local-ID junction rows are preserved as monitor-only snapshots rather than disappearing from Export.
- Split web Export/Import into durable CiviCRM SQL Queue work units. Export now uses prepare, handler/provider staging, metadata, final active-state verification + publish, per-handler baseline, and completion units. Import uses complete preflight, create/update units, delete-missing units, baseline, and completion while preserving the all-preflight-before-write and all-create/update-before-any-delete barriers.
- Added a persistent private per-job workspace and compact staged-file index outside the managed YAML tree. Retry-safe staging work is keyed/idempotent so a lost HTTP response cannot double-count work or duplicate staged output. Mutating work remains fail-closed.
- Added one-pass provider scanning with a disk-backed JSON-lines spool for high-volume CiviRules and extension-provider exports. Identity multiplicities are computed during the provider read, then rows are replayed from disk to build YAML instead of issuing the same provider query twice.
- Reduced repeated YAML parsing by capturing compact identity/hash/name/dependency/provider metadata when a document is staged. Extension indexes and reverse-dependency links are rebuilt primarily from that compact metadata.
- Added durable `publish-state.json` rollback intent. If a PHP worker is killed during live YAML publication, the next reviewed queue callback restores overwritten/deleted YAML and removes newly-created partial files before blocking the indeterminate job; `manifest.yml` remains the last publication marker.
- Improved WordPress queue responsiveness by releasing the PHP session lock before `runNext(FALSE)`. Progress is now honest: named configuration scope + real phase, processed-record counts, heartbeat age, and reconnect guidance. Unknown totals no longer display fabricated percentages or misleading heterogeneous `Step X of Y` progress.
- Extended persistent operation schema with work-unit payload/order/retry safety and honest phase/item progress fields. Existing alpha62 job tables are upgraded idempotently via `information_schema` column checks.
- Added alpha63 PHPUnit ambiguity/workspace regressions, dependency-free architecture contracts, a 10,000-row disk-spool + 5,000-staged-file low-memory stress/recovery gate, and a real-site scenario covering ambiguity, queue reconnect, crash recovery, large-site progress, and zero-diff repeat export.
- No PHP memory-limit increase is performed. Existing transactional Export, optimistic concurrency, CSRF, selected-scope, upload, self-ignore, and Import preflight/delete barriers remain in force.

> **Alpha63 execution boundary:** retry-safe read/stage/baseline work units may replay only after the worker lock proves the prior worker is gone. A live CiviCRM mutation with no durable terminal result is blocked rather than blindly retried. Final Export safety verification and YAML publication intentionally remain adjacent in one non-retry work unit so active configuration cannot change in a gap between verification and publish.

## 0.1.0-alpha62-core

- Same-version Test Case 1 stress hotfix: corrected the 5,000-file YAML stress fixture so each YAML `name` exactly matches its zero-padded filename (`item-00001.yml` -> `name: item-00001`). The previous fixture wrote `name: item-1`, which passed only under the dependency-free filename stub and correctly failed when the real Symfony YAML parser was installed. No production YAML iterator behavior changed.
- Same-version Test Case 1 hotfix: restored protected `api4Get()` override compatibility for custom/test handlers, reused preloaded provider identity rows before live API3 duplicate checks, restored deterministic materialized EntityDefinition export ordering, fixed selected-scope streamed archive matching to honor current selectors plus manifest aliases, corrected the 5k YAML stress assertion for real Symfony/ext-yaml parser output, and aligned the partial-provider regression test with alpha62's all-or-nothing staged-export invariant.
- Reworked large-site execution around true API4/YAML iterators. Core high-volume handlers stream export/import/validation work instead of collecting the whole site into one PHP array, while legacy handler APIs remain compatibility wrappers. API4 scans use an ascending numeric-ID keyset cursor when that cursor is explicitly safe, with guarded bounded offset paging for other providers.
- Replaced whole-export mutation with a staged workspace and rollback journal. Duplicate relative YAML paths block publication; stale files move only after a successful complete stage; `manifest.yml` publishes last; a failed publish restores the previous coherent snapshot.
- Added optimistic active-snapshot fingerprints. Export rechecks CiviCRM immediately before publish, and Import rechecks active configuration after preflight and before delete-missing so concurrent/manual edits fail closed instead of being silently overwritten.
- Added one Configuration Manager operation lock per sync root using CiviCRM's lock manager, covering Export, Import, and Revert.
- Redesigned CiviRules ownership and identity handling. The dedicated CiviRules handler is the single owner of CiviRules provider data; generic extension discovery no longer emits API3/API4 duplicates. Duplicate or otherwise unproven identities are explicitly non-portable/ambiguous and remain backup/monitor-only for automatic CRUD/delete safety.
- Added global staged-path uniqueness protection and collision-safe internal occurrence keys so duplicate semantic identities cannot silently overwrite one another.
- Reworked Synchronize into a compact identity/hash summary scan for streaming handlers. The UI renders bounded pages of 100 summaries and loads exact field-level Details/Ignore choices on demand for one object rather than pre-rendering every full diff.
- Hardened manual admin mutations with a session-bound Configuration Manager CSRF key on every POST action. Read-only synchronization/status views remain accessible without a mutation token.
- Hardened YAML/ZIP upload ownership so managed paths must resolve to `manifest.yml` or a currently registered built-in/custom handler, in addition to the existing traversal, symlink, member-count, per-file, and total-size protections.
- Added persistent operation metadata (`civicrm_civicfg_job` / `civicrm_civicfg_job_item`) and CiviCRM SQL Queue-backed browser orchestration. Export/Import progress is server-derived and persists phase, counters, processed items, handler, heartbeat, and memory telemetry; the old fake/bouncing progress animation is removed. Browser refresh can reconnect to the same active job.
- Queue advancement is serialized by a dedicated worker lock. A completed durable item is retry-idempotent; an indeterminate previously-running mutating item is blocked rather than blindly replayed. This alpha intentionally fails closed here until execution is split into finer persisted cursor tasks.
- Added structured `civicfg` progress/failure logging without raw YAML, credentials, message bodies, or other secret payloads.
- Added dependency-free alpha62 architecture contracts and a `composer qa:stress` low-memory gate. The stress harness exercises 5,000 API4 rows, 5,000 YAML files, duplicate-path blocking, and staged-publish rollback under `memory_limit=256M`.
- No PHP memory-limit increase is performed by the extension.

> **Alpha limitation:** the persistent web job currently queues one bounded-memory Export/Import execution item. Persistent progress/reconnect and fail-closed retry safety are implemented, but automatic fine-grained cursor resume after a killed mutating worker is deliberately not claimed in alpha62; an indeterminate worker blocks and requires a fresh reviewed operation.
...
- Universal Config Ignore/documentation hotfix: Configuration Manager now uses one Config Ignore rule list for both whole-file patterns (`path.yml`) and field-level rules (`path.yml:dot.path`). `extensions/civi.config.manager.yml` remains a mandatory self-protection rule, and the proven volatile values `extensions/*/api3/Job/*.yml:item.last_run`, `extensions/*/api3/Job/*.yml:item.last_run_end`, `scheduled-jobs/*.yml:item.scheduled_run_date`, and `site-tokens/*.yml:item.modified_date` are always active field-level rules. Existing `civicfg_ignore_values` data remains readable and is migrated into the unified setting on save. README and architecture documentation were rewritten as concise current-state documentation, with extended maintainer detail moved to `docs/DETAILED_REFERENCE.md`. No version bump.
